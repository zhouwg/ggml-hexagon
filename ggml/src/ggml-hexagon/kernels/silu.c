...
static inline float silu_f32(float x) {
    ...
    float sx = (x > 18.0f) ? 18.0f : (x < -18.0f ? -18.0f : x);
    float s = 1.0f / (1.0f + expf(-sx));
    return x * s;  // SiLU = x * sigmoid(x)
}

static void silu_thread_func(void * data) {
    silu_thread_data_t * tdata = (silu_thread_data_t *) data;
...
static int ggmlop_dsp_silu_singlethread(remote_handle64 h, const ggml_tensor * src0, ggml_tensor * dst) {
    GGMLHEXAGON_LOG_DEBUG("enter %s", __func__ );
    ggml_compute_forward_silu_f32(src0, dst);
    GGMLHEXAGON_LOG_DEBUG("leave %s", __func__ );
    return 0;
}
...
static int ggmlop_dsp_silu_multithread(remote_handle64 h, const ggml_tensor * src0, ggml_tensor * dst) {
    ...
    if (num_threads <= 1 || n < num_threads * 512) {
        return ggmlop_dsp_silu_singlethread(h, src0, dst);
    }

    worker_synctoken_t synctoken;
    worker_pool_synctoken_init(&synctoken, num_threads - 1);

    silu_thread_data_t tdata[num_threads];
    ...
    silu_thread_func(&tdata[num_threads - 1]);
...
int ggmlop_dsp_silu(remote_handle64 h, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    GGMLHEXAGON_LOG_DEBUG("enter %s\n", __func__);
    char tempbuf[256];
    ggmlhexagon_get_opkey(GGML_OP_UNARY, src0, NULL, tempbuf, 256);

    int64_t begin_time = ggml_time_us();
    if (ggmlop_get_thread_counts() > 1) {
        ggmlop_dsp_silu_multithread(h, src0, dst);
    } else {
        ggmlop_dsp_silu_singlethread(h, src0, dst);
    }
