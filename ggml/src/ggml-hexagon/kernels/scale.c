...
static void scale_thread_func(void * data) {
    ...
    if (src0->type == GGML_TYPE_F16) {
        for (int64_t i = start_idx; i < end_idx; ++i) {
            ...
            *(uint16_t *)((uint8_t *)dst->data + offd) = ggml_compute_fp32_to_fp16(f * scale + bias);
        }
    } else {
        ...
        if (contig) {
            ...
            if (bias == 0.0f) {
                ...
                for (int64_t i = start_idx; i < end_idx; ++i) {
                    dst_ptr[i] = src0_ptr[i] * scale + bias;
                }
            }
        } else {
            if (bias == 0.0f) {
                ...
                for (int64_t i = start_idx; i < end_idx; ++i) {
                    ...
                    *(float *)((uint8_t *)dst->data + offd) = *(const float *)((const uint8_t *)src0->data + off0) * scale + bias;
                }
            }
        }
    }
...
static int ggmlop_dsp_scale_singlethread(remote_handle64 h, const ggml_tensor * src0, ggml_tensor * dst, float scale, float bias) {
    GGMLHEXAGON_LOG_DEBUG("enter %s", __func__ );
    ggml_compute_forward_scale_f32(src0, dst, scale, bias);
    GGMLHEXAGON_LOG_DEBUG("leave %s", __func__ );
    return 0;
}

static int ggmlop_dsp_scale_multithread(remote_handle64 h, const ggml_tensor * src0, ggml_tensor * dst, float scale, float bias) {
    GGMLHEXAGON_LOG_DEBUG("enter %s", __func__ );
    ...
    if (num_threads <= 1 || n < num_threads * 512) {
        return ggmlop_dsp_scale_singlethread(h, src0, dst, scale, bias);
    }
...
int ggmlop_dsp_scale(remote_handle64 h, const ggml_tensor * src0, ggml_tensor * dst) {
    ...
    // op_params[0] = scale, op_params[1] = bias
    float scale;
    float bias;
    memcpy(&scale, dst->op_params, sizeof(float));
    memcpy(&bias, (const float *)dst->op_params + 1, sizeof(float));
    GGMLHEXAGON_LOG_INFO("SCALE: scale=%.8f bias=%.8f, src0 ne=[%lld,%lld,%lld,%lld]",
                         scale, bias, (long long)src0->ne[0], (long long)src0->ne[1],
