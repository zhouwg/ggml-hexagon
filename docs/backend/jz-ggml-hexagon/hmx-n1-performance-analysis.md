...
### 5.1 优先级最高：N=1时自动切换到HVX路径

在`ggmlop_dsp_mulmat`中，根据N值自动选择算法：

```
当 N <= 32 时：使用HVX路径（algotype=0/33）
  - 无tile浪费
  - Q4_0 dot product指令直接处理，无需FP16 tile转换
  - VTCM可全用于权重缓存

当 N > 32 时：使用HMX路径（algotype=30/32）
  - Pipeline模式启用，4级流水线重叠执行
  - HMX tile利用率高
  - 多线程dequant + HMX异步队列
```

实现位置：[mulmat.c:5246](../ggml/src/ggml-hexagon/kernels/mulmat.c#L5246) `ggmlop_dsp_mulmat`函数

### 5.2 优先级次高：ION batch内op融合
...
1. 测试algotype=0（HVX多线程）在N=1时的推理性能
2. 测试algotype=33（HVX+VTCM+多线程）在N=1时的推理性能
3. 对比HVX路径与HMX路径的decode性能
4. 实现N值自动切换逻辑并验证
5. 验证PP阶段（N>32）HMX路径性能不受影响
